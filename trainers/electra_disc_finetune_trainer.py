...
import os
from models import ElectraDiscriminator, ElectraFineTuner
from transformers import ElectraConfig, ElectraForSequenceClassification
import tqdm
import pdb


class DiscriminatorFinetuneTrainer:
    """
    ELECTRATrainer make the pretrained ELECTRA model
    """

    def __init__(self, electra: ElectraFineTuner, vocab_size: int,
                 train_dataloader: DataLoader, train_orig_dataloader: DataLoader, test_dataloader: DataLoader = None,
                 val_dataloader: DataLoader = None,
                 lr: float = 1e-4, betas=(0.9, 0.999), weight_decay: float = 1e-5, warmup_steps=2000,
        ...
        if use_static_embeddings:
            ...
            num_encoder_layers = len(
                self.electra.module.discriminator.electra.encoder.layer) if self.hardware == "parallel" else len(
                self.electra.discriminator.electra.encoder.layer)
        ...
        if self.optim == 'adam':
            ...
            self.optim = AdamW([param for param in self.electra.parameters() if param.requires_grad == True], lr=lr,
                               betas=betas, weight_decay=weight_decay)
            self.scheduler = get_constant_schedule_with_warmup(self.optim, warmup_steps, )
        elif self.optim == 'sgd':
            print("USING SGD OPTIMIZER, LR: {}".format(lr))
            self.optim = SGD([param for param in self.electra.parameters() if param.requires_grad == True], lr=lr,
                             momentum=0.9)
        ...
        if log_file:
            ...
            with open(self.log_file, "w+") as f:
                f.write(
                    "EPOCH,MODE, AVG LOSS, POSITIVE LOSS, NEGATIVE LOSS, TOTAL CORRECT, TOTAL ELEMENTS, ACCURACY, AUC, AUPR, TOTAL POSITIVE CORRECT, TOTAL POSITIVE, ACCURACY\n")
    ...
    def get_embeddings(self, data_loader, multi=False):
        self.electra.eval()
        return self.iteration(0, data_loader, False, "test", multi, return_embeddings=True)

    def iteration(self, epoch, data_loader, train, str_code, multi=False, return_embeddings=False):
        """
        ...
        for i, data in data_iter:
            ...
            if return_embeddings:
                unweighted_loss, scores, batch_embeddings = self.electra.forward(data["electra_input"], mask,
                                                                                 data["electra_label"],
                                                                                 frequencies=data[
                                                                                     "species_frequencies"],
                                                                                 return_embeddings=True)
                embeddings += batch_embeddings.detach().cpu().numpy().tolist()
            else:
                unweighted_loss, scores = self.electra.forward(data["electra_input"], mask, data["electra_label"],
                                                               frequencies=data["species_frequencies"])
            ...
            if train:
                ...
                loss.backward()
                if not self.use_static_embeddings:
            ...
            all_indices.extend(data["index"].detach().cpu().numpy().tolist())
            ...
            if self.loss_func == 'mse':
                ...
                # pdb.set_trace()
                pos_loss += self.class_track_loss(pos_scores[:, 1],
                                                  data["electra_label"][positive_inds].float()).sum().item()
                neg_loss += self.class_track_loss(neg_scores[:, 1],
                                                  data["electra_label"][negative_inds].float()).sum().item()
            # for cross entropy
            elif self.loss_func == 'ce':
            ...
            total_positive_correct += torch.sum(
                predictions[positive_inds] == data["electra_label"][positive_inds]).item()
            ...
            if i % self.log_freq == 0:
                if total_positive > 0:
                    data_iter.write(
                        "epoch: {}, iter: {}, avg loss: {},accuracy: {}/{}={:.2f}%,pos accuracy: {}/{}={:.2f}%, loss: {}".format(
                            epoch, i, cumulative_loss / (i + 1), total_correct, total_samples,
                            total_correct / total_samples * 100, total_positive_correct, total_positive,
                            total_positive_correct / total_positive * 100, log_loss))
                else:
                    data_iter.write(
                        "epoch: {}, iter: {}, avg loss: {},accuracy: {}/{}={:.2f}%,pos accuracy: 0/0, loss: {}".format(
                            epoch, i, cumulative_loss / (i + 1), total_correct, total_samples,
        ...
        if not multi:
            ...
            aupr_score = ELECTRATrainer.calc_aupr(torch.cat(all_labels).flatten().numpy(),
                                                  torch.cat(all_scores).numpy())

        if str_code == "train":
            self.train_scores.append([score.item() for tensor in all_scores for score in tensor])
            self.train_labels.append(torch.cat(all_labels).flatten().numpy())
            self.train_indices.append(all_indices)
        elif str_code == "test":
            self.test_scores.append([score.item() for tensor in all_scores for score in tensor])
            self.test_labels.append(torch.cat(all_labels).flatten().numpy())
        elif str_code == "val":
            self.val_scores.append([score.item() for tensor in all_scores for score in tensor])
            self.val_labels.append(torch.cat(all_labels).flatten().numpy())

        print("EP{}_{}, avg_loss={}, accuracy={:.2f}%".format(epoch, str_code, cumulative_loss / len(data_iter),
                                                              total_correct / total_samples * 100))
        if hasattr(self, 'log_file') and self.log_file:
            with open(self.log_file, "a") as f:
                f.write(
                    "{},{},{},{},{},{},{},{},{},{},{},{},{}\n".format(epoch, str_code, cumulative_loss / len(data_iter),
                                                                      pos_loss / total_positive,
                                                                      neg_loss / (total_samples - total_positive),
                                                                      total_correct, total_samples,
                                                                      total_correct / total_samples * 100, auc_score,
                                                                      aupr_score, total_positive_correct,
                                                                      total_positive,
    ...
    def save(self, epoch, file_path):
        ...
        if self.use_static_embeddings:
            if self.hardware == "parallel":
                if hasattr(self.electra.module, 'projection'):
                    torch.save(self.electra.module.projection.state_dict(), output_file_path + "_projection")
                torch.save(self.electra.module.discriminator.classifier.state_dict(),
                           output_file_path + "_classification_head")
            else:
                if hasattr(self.electra, 'projection'):
                    torch.save(self.electra.projection.state_dict(), output_file_path + "_projection")
                torch.save(self.electra.discriminator.classifier.state_dict(),
                           output_file_path + "_classification_head")
        else:
            if self.hardware == "parallel":
                # pdb.set_trace()
