...
class ELECTRATrainer:
    ...
    def __init__(self, electra: ElectraDiscriminator, vocab_size: int,
                 ...
                 freeze_embed=0,freeze_encoders=0,class_weights=None,loss_func='mse',optim='sgd',hidden_size=None,use_static_embeddings=False):
        """
    ...
    def iteration(self, epoch, data_loader, train,str_code,multi=False,return_embeddings=False):
        ...
        for i, data in data_iter:
            ...
            if return_embeddings:
                unweighted_loss, scores, batch_embeddings = self.electra.forward(data["electra_input"],mask,data["electra_label"],frequencies=data["species_frequencies"],return_embeddings=True)    
                embeddings += batch_embeddings.detach().cpu().numpy().tolist()
            else:
                unweighted_loss,scores = self.electra.forward(data["electra_input"],mask,data["electra_label"],frequencies=data["species_frequencies"])            
    ...
    def save(self, epoch, file_path):
        ...
        if self.use_static_embeddings:
            if self.hardware == "parallel":
                ...
                torch.save(self.electra.module.discriminator.classifier.state_dict(), output_file_path + "_classification_head")
            else:
                if hasattr(self.electra, 'projection'):
                    torch.save(self.electra.projection.state_dict(), output_file_path + "_projection")
                torch.save(self.electra.discriminator.classifier.state_dict(), output_file_path + "_classification_head")     
        else:
            if self.hardware == "parallel":
                #pdb.set_trace()
