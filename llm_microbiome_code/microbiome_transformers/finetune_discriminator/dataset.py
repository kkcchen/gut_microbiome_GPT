...
class ELECTRADataset(Dataset):
    def __init__(self, samples, embedding_path,labels):
        ...
        self.data_indices = np.arange(len(self.samples))
    ...
    def __getitem__(self, item):
        ...
        if self.input_dropout > 0:
            # dropout a percent of the input and move the rest to the left for both electra_input and frequencies
            # Ensure CLS token is not dropped out
            mask = np.random.rand(len(electra_input)) > self.input_dropout
            mask[0] = True  # Always keep the CLS token (first element)
            
            electra_input = electra_input[mask]
            frequencies = frequencies[mask]
            
            # Pad the arrays back to original length
            pad_length = self.seq_len - len(electra_input)
            electra_input = np.pad(electra_input, (0, pad_length), constant_values=self.padding_index)
            frequencies = np.pad(frequencies, (0, pad_length), constant_values=0)
